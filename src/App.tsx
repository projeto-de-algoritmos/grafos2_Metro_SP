...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const nodes = new DataSet(
      estacoesData.map((estacao, index) => ({
        id: index,
        label: estacao.nome,
        title: `Linha: ${estacao.linha}`,
        group: estacao.linha,
      }))
    );

    const edges = new DataSet(
      estacoesData.slice(0, -1).map((estacao, index) => ({
        id: index,
        from: index,
        to: index + 1,
        label: estacao.linha,
    ...
    const options = {
      ...
      groups: {
        'Linha 1 - Azul': {
          color: { background: '#00B5E2' },
        },
        'Linha 2 - Verde': {
          color: { background: '#A9DF6E' },
        },
        'Linha 3 - Vermelha': {
          color: { background: '#E74C3C' },
        },
        'Linha 4 - Amarela': {
          color: { background: '#F4D03F' },
        },
        'Linha 5 - Lilás': {
